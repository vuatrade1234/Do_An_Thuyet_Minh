...
public partial class MapPage : ContentPage
{
    ...
    {
        LblNoPoi.Text = "Kéo xuống / \n" + _loc.Get("no_poi_in_radius");
        CancelRouteBtn.Text = "✕ " + _loc.Get("cancel_route");
        BtnRoute.Text = "🧭 " + _loc.Get("detail_navigate");
        BtnDetail.Text = "› " + _loc.Get("poi_btn_detail");
        BtnAudio.Text = "🎧 " + _loc.Get("poi_btn_audio");
        LblPoiSubtitle.Text = _loc.Get("poi_local_attraction");
    }
    ...
    private async void OnToggleLanguageClicked(object sender, EventArgs e)
    {
        ...
        System.Diagnostics.Debug.WriteLine($"[Lang] chosen='{chosen}'");

        if (string.IsNullOrEmpty(chosen) || chosen == "✕") return;

        var selected = LocalizationService.LocaleInfo
    .Where(kv => chosen.Contains(kv.Value.NativeName))
    .Select(kv => (AppLocale?)kv.Key)
    .FirstOrDefault();

        if (selected.HasValue)
            _loc.SetLocale(selected.Value);
    }
    ...
    private void OnClosePoiPanelClicked(object sender, EventArgs e)
    {
        _vm.SelectedPoi = null;
        _vm.ShowPoiPanel = false;
        UpdatePoiPanel();
    }
