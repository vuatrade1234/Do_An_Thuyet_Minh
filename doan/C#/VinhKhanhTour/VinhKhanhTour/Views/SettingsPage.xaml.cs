...
public partial class SettingsPage : ContentPage
{
    ...
    private void RefreshLocalizedText()
    {
        ...
        // Cập nhật nhãn chọn tốc độ (Speed labels)
        BtnSlow.Text = "🐢 " + _loc.Get("slow_label");
        BtnNormal.Text = "🚶 " + _loc.Get("normal_label");
        BtnFast.Text = "🚴 " + _loc.Get("fast_label");
        BtnVeryFast.Text = "🚀 " + _loc.Get("very_fast_label");

        var (sEmoji, sText) = SpeechRate switch
        {
            < 0.8f => ("🐢", _loc.Get("slow_label")),
            < 1.2f => ("🚶", _loc.Get("normal_label")),
            < 1.6f => ("🚴", _loc.Get("fast_label")),
            _ => ("🚀", _loc.Get("very_fast_label"))
        };
        LblSpeechRate.Text = $"{sEmoji} {sText}";
