...
public class PoiModel
{
    ...
    // ── Audio URLs từ GCS (upload qua CMS) ───────────────────────────
    // Key = "vi" | "en" | "zh" | "ja" | "ko"
    // Value = https://storage.googleapis.com/...
    public Dictionary<string, string> AudioUrls { get; set; } = new();

    // ── Computed: URL audio theo ngôn ngữ hiện tại ───────────────────
    public string? LocalizedAudioUrl
    {
        get
        {
            var loc = Services.LocalizationService.Instance;
            var lang = loc.CurrentLocale switch
            {
                Services.AppLocale.Vietnamese => "vi",
                Services.AppLocale.English => "en",
                Services.AppLocale.Chinese => "zh",
                Services.AppLocale.Japanese => "ja",
                Services.AppLocale.Korean => "ko",
                _ => "vi"
            };

            // Chỉ trả audio URL khi ĐÚNG ngôn ngữ hiện tại
            // Nếu không có → trả null → hệ thống sẽ dùng TTS theo ngôn ngữ đó
            if (AudioUrls.TryGetValue(lang, out var url) && !string.IsNullOrEmpty(url))
                return url;
            return null;
        }
    }
