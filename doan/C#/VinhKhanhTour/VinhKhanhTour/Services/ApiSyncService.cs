...
public class ApiSyncService
{
    ...
    private List<PoiModel> ConvertFromApi(JsonArray nodes)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            // Map AudioUrls dictionary từ API
            var audioNode = node["audioUrls"];
            if (audioNode != null)
            {
                foreach (var kvp in audioNode.AsObject())
                {
                    var val = kvp.Value?.ToString();
                    if (!string.IsNullOrEmpty(val))
                        poi.AudioUrls[kvp.Key] = val;
                }
            }
    ...
    public async Task LogPoiPlayAsync(string poiId, string poiName, string lang, int duration = 0, double lat = 0, double lng = 0)
    {
        await LogHistoryAsync("play_poi", poiId, poiName, lang, duration);
        await LogAnalyticsEventAsync("poi_play", poiId, lang, duration, lat, lng);
    }
    ...
    // ── Core ghi analytics → api/analytics ───────────────────────────
    private async Task LogAnalyticsEventAsync(string eventType, string poiId, string lang, int duration, double lat, double lng)
    {
        try
        {
            var body = new
            ...
            var res = await _http.PostAsJsonAsync("api/analytics", body);
            System.Diagnostics.Debug.WriteLine(res.IsSuccessStatusCode
                ? $"[Analytics] OK → {eventType} {poiId}"
                : $"[Analytics] Fail: {res.StatusCode}");
        }
        catch (Exception ex)
        {
